# TODO — 发布前接口保护与双仓关系核对（2026-09-08）

- [x] 对比独立仓与 Black-God/shuyu：后者已包含 v4.1 新接口和测试，禁止用旧版覆盖。
- [x] 为私有状态路由加入 OWNER_TOKEN 默认拒绝鉴权；保护 /status、/talk、/interpret、/broadcast 和读取状态的 /coin 分支。
- [x] 补齐鉴权、状态隔离与定时行为测试；Node 32 项、Python 9 项全部通过。
- [x] 更新部署说明与版本关系；本轮结果归档至 docs/done/2026-09-08-worker-auth.md，保留下面的既有历史任务。

本轮仅完成本地修复与验证，未部署 Worker、未设置线上密钥，未迁移或覆盖 v4.1 引擎。
