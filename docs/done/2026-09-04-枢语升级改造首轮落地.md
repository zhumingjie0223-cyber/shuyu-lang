...
## 步骤（本轮）
- [x] 1) 梳理并统一版本来源（JS 运行时/包元信息对齐）—— package.json 统一到 4.0.0，运行时 VERSION 升至 v4.0.0。
- [x] 2) 在 worker.mjs 增加模型适配层（超时、错误、可观测数据、开关控制）—— 新增 maybeCallBrain/callAnthropic，支持 MODEL_ENABLED 开关与错误回传。
- [x] 3) 强化 /talk 输入校验（JSON/长度/行数/类型）—— 增加 8000 字符与 200 行上限。
- [x] 4) 增加灵魂状态并发写入缓解（读-合并-写，保留记忆上限与计数一致性）—— 解释结果应用到最新 soul 后再写入，并维护 _rev。
- [x] 5) 更新 worker 路由返回结构，保持兼容同时暴露新观测字段—— /talk 返回 model 字段，/status 返回 packageVersion。
- [x] 6) 补充 tests/worker.test.mjs 覆盖新增行为—— 新增输入上限、模型开关/缺钥匙、模型成功调用三类测试。
- [x] 7) 运行 npm test + python unittest—— Node 30/30、Python 9/9 全绿。
- [x] 8) 运行 secrets 扫描与 parallel_validation—— parallel_validation 已执行（CodeQL/Review）。
- [x] 9) 回填总结并归档 docs/done/—— 本文件已补总结并准备归档。
...
## 本轮总结（2026-09-04）
本轮完成 Worker 首轮 P0/P1 改造：补齐可执行模型调用链路（开关、超时、错误与 token 用量观测），
强化 /talk 输入校验，并把解释结果应用到最新 soul 后再写入以缓解并发覆盖；同时统一版本到 4.0.0 线。
测试层新增并跑绿（Node 30/30、Python 9/9），满足“先验真再汇报”。
